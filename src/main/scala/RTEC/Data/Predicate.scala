...
sealed abstract class Predicate(val head: HeadClause, val body: Seq[BodyClause]) {
    ...
    // Parses the set of entities applied and returns a series of variable assignments to be filtered through the body clauses
        ...
        entities.view
            .map {entity =>
                ...
                if (conflict)
                    ...
                    (entity, entityDict, Map.empty[String, Intervals], Map.empty[String, Set[Int]])
                }
            }
            .filter(_ != null)
    }
